...
- **Interactive Editor**: Drag-and-drop placeholders for Names, Dates, and IDs. Full control over fonts and colors.
- **Dual Export Modes**: Choose between Protected (Image PDF) or Interactive (Selectable PDF).
- **ZIP Download**: Automatically packages all individual PDFs into a single ZIP file.
...
### 🔒 Privacy Statement
**Zero Data Transfer**: This application does not have a backend. Your certificates and recipient data are processed locally using JavaScript. We do not track, store, or see your data.

---

## 繁體中文 (出Cert神器-零幀出Cert)
...
- **批量處理**: 通過快速粘貼（換行分隔）或 TSV/CSV 文件導入收件人列表。
- **交互式編輯器**: 拖放姓名、日期和編號的佔位符。完全控制字體和顏色。
- **雙導出模式**: 在圖片型 PDF（快速安全）或文字型 PDF（可選取文字）之間選擇。
- **ZIP 下載**: 自動將所有個人 PDF 打包成一個 ZIP 文件。

### 🚀 如何運行
1. 下載或克隆此倉庫。
2. 雙擊 `index.html` 文件在瀏覽器中打開。
3. 無需安裝，無需聯網！

### 🔒 隱私聲明
**零數據傳輸**: 此應用程序沒有後端。您的證書和收件人數據使用 JavaScript 在本地處理。我們不會跟踪、存儲或查看您的數據。

---

## 简体中文 (出Cert神器-零帧出Cert)
...
### 🔒 隐私声明
**零数据传输**: 此应用程序没有后端。您的证书和收件人数据使用 JavaScript 在本地处理。我们不会跟踪、存储或查看您的数据。
...
- **一括処理**: クイックペーストまたは CSV/TSV ファイルで受信者リストをインポート。
- **インタラクティブエディタ**: 名前、日付、IDのプレースホルダーをドラッグ＆ドロップ。フォントや色を自由に調整可能。
- **2つの書き出しモード**: 画像PDF（高速・安全）またはテキストPDF（選択可能）を選択可能。
- **ZIPダウンロード**: すべてのPDFを自動的に1つのZIPファイルにまとめます。

### 🚀 実行方法
1. このリポジトリをダウンロードまたはクローンします。
2. `index.html` ファイルをダブルクリックしてブラウザで開きます。
3. インストールやインターネット接続は不要です！

### 🔒 プライバシーポリシー
**データ転送ゼロ**: このアプリにはバックエンドがありません。証明書と受信者データはJavaScriptを使用してローカルで処理されます。データの追跡、保存、閲覧は一切行いません。
...
- **배치 처리**: 빠른 붙여넣기 또는 CSV/TSV 파일을 통해 수신자 목록을 간편하게 가져옵니다.
- **대화형 편집기**: 이름, 날짜, ID 자리 표시자를 드래그 앤 드롭으로 배치하세요. 글꼴과 색상을 완벽하게 제어할 수 있습니다.
- **듀얼 내보내기 모드**: 보안 중심의 이미지 PDF 또는 텍스트 선택이 가능한 PDF 중 선택할 수 있습니다.
- **ZIP 다운로드**: 생성된 모든 개별 PDF를 하나의 ZIP 파일로 자동 압축합니다.

### 🚀 실행 방법
1. 이 저장소를 다운로드하거나 클론합니다.
2. `index.html` 파일을 더블 클릭하여 브라우저에서 엽니다.
3. 설치나 인터넷 연결이 필요 없습니다!

### 🔒 개인 정보 보호 선언
**데이터 전송 제로**: 이 애플리케이션은 백엔드가 없습니다. 인증서와 수신자 데이터는 JavaScript를 사용하여 로컬에서 처리됩니다. 당사는 귀하의 데이터를 추적, 저장 또는 열람하지 않습니다.
...
- **Procesamiento por Lotes**: Importa listas de destinatarios mediante Pegado Rápido o archivos CSV/TSV.
- **Editor Interactivo**: Arrastra y suelta marcadores de posición para Nombres, Fechas e IDs. Control total sobre fuentes y colores.
- **Modos de Exportación Dual**: Elige entre PDF de Imagen (Rápido y Seguro) o PDF Interactivo (Texto Seleccionable).
- **Descarga en ZIP**: Empaqueta automáticamente todos los PDF individuales en un solo archivo ZIP.

### 🚀 Cómo Ejecutar
1. Descarga o clona este repositorio.
2. Haz doble clic en el archivo `index.html` para abrirlo en tu navegador.
3. ¡Sin instalación ni necesidad de internet!
...
- **المعالجة الجماعية**: استيراد قوائم المستلمين عبر اللصق السريع أو ملفات CSV/TSV.
- **محرر تفاعلي**: سحب وإفلات العناصر النائبة للأسماء والتواريخ والأرقام التعريفية. تحكم كامل في الخطوط والألوان.
- **وضع التصدير المزدوج**: اختر بين ملف PDF صورة (آمن) أو ملف PDF تفاعلي (نص قابل للتحديد).
- **تحميل ZIP**: يقوم تلقائياً بتجميع كافة ملفات PDF الفردية في ملف ZIP واحد.

### 🚀 كيفية التشغيل
1. قم بتحميل أو استنساخ هذا المستودع.
2. انقر نقراً مزدوجاً على ملف `index.html` لفتحه في متصفحك.
3. لا يتطلب تثبيت أو اتصال بالإنترنت!
